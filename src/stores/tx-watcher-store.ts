...
interface TxWatcherActions {
  /** Register a new transaction for global watching. Idempotent. Pass originalRegisteredAt when rehydrating from localStorage to preserve TTL. */
  register: (txHash: string, txType: string, toastConfig: TxToastConfig, originalRegisteredAt?: number) => void;
  ...
  /** Update the user alias for scoped localStorage keys */
  updateAlias: (alias: string | null) => void;
...
const TX_STORAGE_PREFIX = "tx-watcher:";
...
/** Write a TX entry to localStorage for crash recovery. Scoped by user alias. */
function persistTx(
  alias: string | null,
  txHash: string,
  txType: string,
  ...
  if (!alias) return; // Can't scope without identity
  try {
    const data: PersistedTx = { txHash, txType, toastConfig, registeredAt };
    localStorage.setItem(
      `${TX_STORAGE_PREFIX}${alias}:${txHash}`,
      JSON.stringify(data)
    );
  } catch {
    // localStorage unavailable (quota exceeded, private browsing) — silent fail
  }
}

/** Remove a single TX entry from localStorage. Scoped by user alias. */
function removeTxFromStorage(alias: string | null, txHash: string): void {
  if (!alias) return; // Can't scope without identity
  try {
    localStorage.removeItem(`${TX_STORAGE_PREFIX}${alias}:${txHash}`);
  } catch {
...
/**
 * Scan localStorage for persisted TX entries scoped to the given alias,
 * filtering out expired ones.
 * Exported so the TxWatcherBridge can call it during rehydration.
 */
export function getPersistedTxs(alias: string | null): PersistedTx[] {
  if (!alias) return []; // Can't scope without identity
  try {
    const now = Date.now();
    const results: PersistedTx[] = [];
    const scopedPrefix = `${TX_STORAGE_PREFIX}${alias}:`;
    ...
    for (let i = localStorage.length - 1; i >= 0; i--) {
      ...
      try {
        ...
        if (!raw) continue;
        const entry = JSON.parse(raw) as PersistedTx;
...
/** Remove all tx-watcher entries from localStorage. */
function clearAllTxStorage(): void {
  try {
    for (let i = localStorage.length - 1; i >= 0; i--) {
      const key = localStorage.key(i);
...
function handleTerminal(
  ...
  // Remove from localStorage — TX is resolved
  removeTxFromStorage(alias, txHash);
...
export const txWatcherStore = createStore<TxWatcherStore>()((set, get) => ({
  ...
  // Actions
  register: (txHash, txType, toastConfig, originalRegisteredAt?: number) => {
    const { transactions, jwt, alias } = get();
    ...
    // Preserve original registeredAt when rehydrating from localStorage
    // to prevent TTL reset across repeated crashes
    ...
    // Persist to localStorage for crash recovery (scoped by alias)
    persistTx(alias, txHash, txType, toastConfig, registeredAt);
  ...
  cleanup: () => {
    ...
    for (const [hash, entry] of next) {
      if (now - entry.registeredAt > MAX_AGE_MS) {
        if (entry._confirmedTimeout) clearTimeout(entry._confirmedTimeout);
        entry._abortController?.abort();
        removeTxFromStorage(alias, hash);
        next.delete(hash);
        changed = true;
      }
    }
