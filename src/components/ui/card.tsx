...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card/50 text-card-foreground flex flex-col gap-4 rounded-lg border border-border/50 py-5 backdrop-blur-sm",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("leading-none font-display font-semibold text-sm", className)}
      {...props}
    />
  )
}
