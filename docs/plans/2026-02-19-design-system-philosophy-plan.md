# Design System Philosophy Layer — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add a foundational `philosophy.md` to the design-system skill and wire it into `SKILL.md`.

**Architecture:** Create one new markdown file with the approved design content (vision, 6 principles, accessibility standards, quick reference card). Update SKILL.md to reference it as the foundation layer — first in the docs table, integrated into Mode 3 Reference, and replacing the old "Key Principles" with the new six.

**Tech Stack:** Markdown files in `.claude/skills/design-system/`

---

### Task 1: Create `philosophy.md`

**Files:**
- Create: `.claude/skills/design-system/philosophy.md`

**Step 1: Write the file**

Create `.claude/skills/design-system/philosophy.md` with the full approved content from the design doc at `docs/plans/2026-02-19-design-system-philosophy-design.md`.
...
```bash
git add -f .claude/skills/design-system/philosophy.md
git commit -m "docs(design-system): add philosophy.md — foundational design principles

New file establishing Andamio's design philosophy: vision (Programmable
Economy of Competence), six principles (Invisible Infrastructure, Clarity
Over Cleverness, Earned Progression, Moments of Commitment, Breathing Room,
Accessibility is Not Optional), WCAG 2.1 AA standards, and quick reference.
...
### Task 2: Wire `philosophy.md` into `SKILL.md`

**Files:**
- Modify: `.claude/skills/design-system/SKILL.md`

**Step 1: Update Mode 3 Key Principles**

Replace the current "Key Principles" section (lines 161-169) which has the old four principles:

```
**Consistency Over Creativity**: All UI follows established patterns...
**Semantic Over Literal**: Use semantic colors...
**Composition Over Configuration**: Build complex UIs...
**Andamio Prefix**: All components use `Andamio` prefix...
```

With a reference to the philosophy doc and a summary of the six principles:
...
**Step 2: Add philosophy.md to the Documentation Files table**

In the Mode 3 "Documentation Files" table (line 173), add `philosophy.md` as the FIRST row:

```markdown
| [philosophy.md](./philosophy.md) | **Start here.** Design vision, six principles, accessibility standards |
```

**Step 3: Add philosophy.md to the "All Documentation Files" table**

In the bottom "All Documentation Files" table (line 222), add `philosophy.md` as the FIRST row:
...
```bash
git add -f .claude/skills/design-system/SKILL.md
git commit -m "docs(design-system): wire philosophy.md into SKILL.md

Replace old four-word principles with six opinionated principles table.
Add philosophy.md as first entry in both documentation tables.
...
1. `philosophy.md` exists and is valid markdown
2. `SKILL.md` references `philosophy.md` in three places (Key Principles, Documentation Files, All Documentation Files)
3. `git diff --stat` shows exactly 2 files: 1 created, 1 modified
