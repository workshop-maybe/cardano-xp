# Design: Design System Philosophy Layer

**Date:** 2026-02-19
**Branch:** `feat/studio-ux-improvements`
**Status:** Approved

## Problem

The design-system skill is 13 files of pure implementation reference — component APIs, CSS architecture, icon mappings, spacing values. Every file is 88-99% "how" with near-zero "why." There is no articulation of design philosophy, UX principles, accessibility standards, or visual hierarchy reasoning. When someone encounters a novel UI situation not covered by an existing component, they have zero guidance.

## Solution

A single foundational document (`philosophy.md`) that the rest of the skill references. Opinionated, Apple HIG-style. Dual audience: human developers and AI agents.
...
- **Project Manager** (the "Gabe Newell") — creates projects, defines tasks with rewards, sets credential prerequisites, manages treasury, reviews submitted work
...
### Principle 1: Invisible Infrastructure

**Stance:** The blockchain is the engine, never the dashboard.
...
**Stance:** Every element answers "what is this?" and "what do I do next?" instantly.
...
### Principle 3: Earned Progression

**Stance:** The UI reflects demonstrated competence, not navigation skill.
...
### Principle 4: Moments of Commitment

**Stance:** The wallet appears only at genuine decision points, and every signing is preceded by understanding.
...
**Stance:** Space is not empty — it creates hierarchy, separates ideas, and signals confidence.
...
**Stance:** Every feature ships accessible. No exceptions, no "we'll add it later."
...
**The Three-Channel Rule:** Every status communicates through *color + icon + text*. A user who can't see color, can't see icons, or can't read small text should still understand the state.
...
A one-page cheat sheet for both humans and AI agents. When making any design decision, run through this.
...
- Create `philosophy.md` as a new file in `.claude/skills/design-system/`
- Update `SKILL.md` to reference `philosophy.md` in the Documentation Files table and in Mode 3 (Reference)
- Add philosophy.md as the FIRST file listed — it's the foundation everything else builds on

## Out of Scope

- No changes to existing implementation docs (spacing.md, layouts.md, etc.) — the philosophy doc stands alone as a foundation layer
- No code changes
